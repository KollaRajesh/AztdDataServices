...
namespace Demo.AzCosmos.EFCore.ConsoleApp.Repositories
{
    public class CollegeCosmosClientRepositoryAsync : IRepositoryAsync<College>
    {
        ...
        public async Task<College> GetByIdAsync(int EntryId)
        {
            var sqlQueryText = $"SELECT * FROM c WHERE c.Id = {EntryId}";
        ...
        public async Task<College> GetByIdAsync(int EntryId, string partitionKey)
        {
            ...
            try
            {
                // Read the item to see if it exists.  
                 entityResponse = await this.container
                                            .ReadItemAsync<College>(EntryId.ToString(),
        ...
        public async Task<bool> IsExists(int EntryId)
        {
             return (await GetByIdAsync(EntryId)) !=null ;
        }

        public async Task<bool> IsExists(int EntryId, string partitionKey)
        {
            return (await GetByIdAsync(EntryId,partitionKey)) !=null ;
        }
